...
const DEFAULT_PACK_DIR = join(__dirname, 'pack')

export interface UiKitIconsPluginOptions {
  /** Куда писать сгенерированные файлы (обязательно) */
  outDir: string
  /** Папка с svg-иконками (по умолчанию — pack/ внутри пакета @dotdev/icons) */
  dir?: string
  ...
  iconsFile?: string
  /** Имя barrel-файля (по умолчанию: index.ts) */
  indexFile?: string
}
...
function renderIndex(iconsFile: string): string {
  return `/**
 * Generated file. Do not edit.
 */

export { icons } from './${iconsFile}'
`
}

export default function uiKitIcons(options: UiKitIconsPluginOptions): Plugin {
  const iconsDir = options.dir ?? DEFAULT_PACK_DIR
  ...
  function regenerate() {
    try {
      ...
      writeIfChanged(join(resolvedOutDir, indexFile), renderIndex(iconsFile))
