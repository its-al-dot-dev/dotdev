...
export function renderVars(registry: Registry, ui?: string): string {
  ...
  if (ui === undefined) {
    for (const entry of registry.primitives()) {
      root.push(
        `  ${entry.varName}: ${emitValue(resolveValue(entry.value, registry))};`,
      );
    }
  }
  ...
  for (const entry of registry.semantics()) {
    ...
    root.push(
      `  ${entry.varName}: ${emitValue(resolveValue(entry.light, registry))};`,
    );
    if (entry.dark != null) {
      dark.push(
        `  ${entry.varName}: ${emitValue(resolveValue(entry.dark, registry))};`,
      );
    }
  }
